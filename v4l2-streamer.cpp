    ...
    bool h264parse = false)
{
  ...
  elements->h264enc = gst_element_factory_make("v4l2h264enc", NULL);
  // elements->h264enc = gst_element_factory_make("avenc_h264_omx", NULL);
  ...
  if (!elements->v4l2src)
  {
    g_printerr("v4l2src failed.\n");
    return 1;
  }

  if (!elements->capsfilter1)
  {
    g_printerr("capsfilter1 failed.\n");
    return 1;
  }

  if (!elements->jpegdec)
  {
    g_printerr("jpegdec failed.\n");
    return 1;
  }
  if (!elements->queue1)
  {
    g_printerr("queue1 failed.\n");
    return 1;
  }
  if (!elements->h264enc)
  {
    g_printerr("h264enc failed.\n");
    return 1;
  }
  if (!elements->queue2)
  {
    g_printerr("queue2 failed.\n");
    return 1;
  }
  if ((h264parse && !elements->h264parse))
  {
    g_printerr("h264parsegi failed.\n");
    return 1;
  }
  if (!elements->capsfilter2)
  {
    g_printerr("capsfilter2 failed.\n");
    return 1;
  }
  if (!elements->appsink)
  {
  ...
  // g_object_set(elements->h264enc, "bitrate", bitrate, "keyframe-period", 300, "format", "YVYU", NULL);
  g_object_set(elements->v4l2src, "do-timestamp", TRUE, NULL);

  g_object_set(elements->v4l2src, "device", "/dev/video0", NULL);
  if (video_device != NULL)
    g_object_set(elements->v4l2src, "device", video_device, NULL);

  g_object_set(elements->capsfilter1, "caps", gst_caps_new_simple("image/jpeg", "width", G_TYPE_INT, width, "height", G_TYPE_INT, height, "framerate", GST_TYPE_FRACTION, framerate, 1, NULL), NULL);

  g_object_set(elements->capsfilter2, "caps", gst_caps_new_simple("video/x-h264", "stream-format", G_TYPE_STRING, "byte-stream", "alignment", G_TYPE_STRING, "au", "profile", G_TYPE_STRING, "constrained-baseline", NULL), NULL);
